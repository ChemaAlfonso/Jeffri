...
export class TelegramMessengerManager implements Messenger {
	...
	async sendMessage(user: User, message: Message): Promise<void> {
		...
		if (!instance) {
			throw new Error(
				`[TelegramMessengerManager] Cannot send message: service not connected ${JSON.stringify(
					message.receiver.id
	...
	private async createAndConnectInstance(user: User): Promise<Messenger> {
		...
		const newInstance = container.get<Messenger>('Chats.TelegramMessenger')
